import uuid, time, requests, json, base64, csv, os
...
def build_summary_item(item):
    ...
    return {
        ...
        "display_label": "Authentication Fee" if is_auth else "Document Price",
        "unit_price": item.get_unit_price(),
        "quantity": item.get_quantity() if not is_auth else 1,
        "base_amount": base_amount,
        "rush_fee": rush_fee,
        "total_amount": base_amount + rush_fee,
        "is_authentication": is_auth,
        "is_rush": item.rush_processing,
        "page_count": item.tor_page_count if not is_auth else None,
        "unit_label": (
            f"{item.tor_page_count} pages x P{item.get_unit_price()}/page"
            if item.tor_page_count and not is_auth
            else None
        ),
    }
...
def send_otp_sms(phone_number, otp_code):
    # HTTP SMS API (Primary OTP provider)
    ...
    try:
        ...
        # Ensure phone number is in international format (+639xxxxxxxxx)
...
"""
                        ...
                        # Send SMS with OTP and check if it was successful
                        sms_sent = send_otp_sms(
                            master_student.phone_number, otp_obj.otp_code
                        )
                        if sms_sent:
        ...
        if action == "submit_request":
            ...
            for base in base_docs:
                ...
                if delivery == "LBC":
                    ...
                    college_floor = (
                        "Computer Arts Technological College - New Main Building"
                    )
                ...
                for dt in to_create:
                    if dt:
                        ...
                        doc_request = DocumentRequest.objects.create(
                            ...
                            shipping_first_name=college_first_name,
                            shipping_last_name=college_last_name,
                            shipping_phone=college_phone,
                            shipping_floor=college_floor,
                            shipping_street=college_street,
                            shipping_province=college_province,
                            shipping_city=college_city,
                            shipping_barangay=college_barangay,
                            shipping_zip=college_zip,
                            shipping_landmark=college_landmark,
...
def registrar_dashboard(request):
    ...
    history = DocumentRequest.objects.filter(
        is_deleted=False, status__in=["COMPLETED", "REJECTED"]
    ).order_by("-created_at")
    ...
    if request.method == "POST":
        ...
        if action == "approve":
            ...
            log_audit(
                request.user, "UPDATE", "DocumentRequest", batch_id, "Rejected batch."
            )
            ...
            if t_no:
                try:
                    ...
                    if result.get("meta", {}).get("code", 200) >= 400:
                        messages.warning(
                            request,
                            f"LBC tracking registration may have failed. Please verify manually.",
                        )
                        print(
                            f"[LBC WARNING] Tracking registration for {t_no}: {result}"
                        )
                except Exception as e:
                    messages.warning(
                        request, f"LBC tracking registration failed: {str(e)}"
                    )
            ...
            if lbc_count > 0:
                messages.success(
                    request,
                    f"{lbc_count} LBC document(s) marked as COMPLETED (LBC will notify student). Tracking: {t_no or 'N/A'}",
                )
            if pickup_count > 0:
                messages.success(
                    request,
                    f"{pickup_count} pickup document(s) marked as READY - waiting for student to claim.",
                )
            log_audit(
                request.user,
                "UPDATE",
                "DocumentRequest",
                batch_id,
                f"Marked as READY. Tracking: {t_no}, Days: {processing_days}",
            )
            ...
            for item in batch:
                ...
                if is_tor_request:
                    ...
                    student_master = StudentMasterList.objects.filter(
                        student_id=item.student.username
                    ).first()
                    ...
                    print(
                        f"[DEBUG] Created TORRequestHistory for student {student_master.student_id if student_master else 'None'}, is_free: {item.get_price() == 0}"
                    )
...
def cashier_dashboard(request):
    ...
    history = DocumentRequest.objects.filter(
        is_deleted=False, status__in=["PROCESSING", "READY", "COMPLETED"]
    ).order_by("-created_at")
    ...
    if request.method == "POST":
        ...
        if action == "confirm_payment":
            ...
            was_online = any(
                item.status == "PENDING_CASHIER_APPROVAL" for item in batch
            )
...
def xendit_webhook(request):
    ...
    if status == "PAID" and ext_id.startswith("BATCH-"):
        ...
        items = DocumentRequest.objects.filter(
            batch_id=batch_id, status__in=["APPROVED", "PAYMENT_REQUIRED"]
        )
        if items.exists():
            new_no = SystemCounter.get_next_receipt_no()
            ...
            CollectionLog.objects.create(
                ...
                student_name=items.first().get_student_name(),
                amount_paid=total,
...
def payment_success(request):
    ...
    request.session.pop("pending_xendit_invoice", None)
    try:
        if request.user.is_authenticated:
...
"""
                ...
                print(f"[API SMS] OTP sent to {master_student.phone_number}")
        except Exception as e:
...
def _get_mock_tracking_data(tracking_num):
    ...
    return {
        ...
        "timeline": [
            ...
            {
                "dateTime": "",
                "location": "Legazpi City Hub",
                "status": "Picked up by LBC Rider",
            },
            {
                "dateTime": "",
                "location": "Legazpi City Hub",
...
def track_lbc_shipment(request, tracking_num):
    ...
    logger.info(
        f"Using mock tracking data for {tracking_num} (LBC API unavailable: {error})"
    )
    mock_data = _get_mock_tracking_data(tracking_num)

    return JsonResponse(
        {"success": True, "data": mock_data, "note": "Demo mode - LBC API unavailable"}
    )
...
def track_and_notify(request, tracking_num):
    ...
    tracking_data = (
        data if (success and data) else _get_mock_tracking_data(tracking_num)
    )
        ...
        try:
            ...
            if doc_request.batch_id:
                ...
                if is_free_tor and batch_items.count() == 1:
                    ...
                    doc_request.tor_price_override = total_price
                    doc_request.status = (
                        "APPROVED"  # Set to APPROVED so student can see PAY button
                    )
                    doc_request.save()
                    ...
                    log_audit(
                        ...
                        "DocumentRequest",
                        str(doc_request.id),
                        f"TOR page count set to {page_count}. Price: ₱{tor_price}",
                    )
...
def pay_with_xendit(request, batch_id):
    batch = DocumentRequest.objects.filter(
        batch_id=batch_id, status__in=["APPROVED", "PAYMENT_REQUIRED", "PROCESSING"]
    )
    if not batch.exists():
    ...
    data = {
        ...
        "amount": float(total),
        "description": f"Payment Batch {batch_id}",
    ...
    try:
        ...
        if res.status_code in [200, 201]:
            ...
            batch.update(payment_reference=invoice_data.get("id"))
